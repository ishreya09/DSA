...
// everything similar to C except that we created struct there

/* Given a reference (pointer to pointer)
to the head of a list and an int, inserts
a new node on the front of the list. */
void push(Node** head_ref, int new_data) // double pointer method 
{
	/* 1. allocate node */
...
void printList(Node *node) // is head not getting changed ? 
// is not assigning to a temp variable a good practice?
{
	while (node != NULL)
	{
		cout<<" "<<node->data;
