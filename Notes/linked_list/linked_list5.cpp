...
struct node { // what is this??
	int info;
	node* link = NULL;
	node() {} // what does this mean? Is struct having a constructor like something here as well
	node(int a) // what does this mean? what is it?
		: info(a)
	{
	}
};
...
void deleteNode(node*& head, int val)
{
	...
	if (head->info == val) {
		...
		delete (t); // Else changes previous node's link to
					// current node's link
                    // is delete similar to free or are they working differently? 
		return;
	}
...
void print(node* head)
{
	...
	// 'true'
	if (head == NULL and cout << endl) // and?? - can we write that?
		return;
	cout << head->info << ' ';
	print(head->link);
}
