...
1. Find max value in a col and in a row and store both in a vector
2. Iterate through the whole 2D vector and check if they are less than  min(maxHor[i],maxVer[j]) where i = row j= col
3. If they are less - take difference and add to ans.
    ...
    int maxIncreaseKeepingSkyline(vector<vector<int>>& grid) {
        ...
        for(int i=0; i<grid.size();i++){
            ...
            for(int j=0;j<grid.size();j++){
                m=max<int>(m,grid[j][i]);
                p=max<int>(p,grid[i][j]);
            }
            maxVer.push_back(m);
            maxHor.push_back(p);
        }
        ...
        for(int i=0;i<grid.size();i++){
            for(int j=0; j<grid.size();j++){
                int k = min<int>(maxHor[i],maxVer[j]);
                if(grid[i][j]<k){
                    ans+=k-grid[i][j];
                    grid[i][j]=k;
                }
