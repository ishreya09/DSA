...
1. Add 1*min(k,numOnes) to sum and decrement k by numsOne. Check if k<=0 - if true return sum
2. Repeat the same for zero and negative 
3. Return sum.
...
public:
    int kItemsWithMaximumSum(int numOnes, int numZeros, int numNegOnes, int k) {
        int sum=0;
        sum+=1*min<int>(k,numOnes);
        k=k-1*numOnes;
        if(k<=0)
            return sum;
        sum+=0*min<int>(k,numZeros);
        k=k-1*numZeros;
        if(k<=0){
            return sum;
        }
        sum+=-1*min<int>(k,numNegOnes);
        k=k-1*numNegOnes;
        if(k<=0){
            return sum;
        }
