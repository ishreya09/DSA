...
5. To move in the array in a circular way- its better to append the starting elements to the end of the array 
...
public:
    int canCompleteCircuit(vector<int>& gas, vector<int>& cost) {
        int idx=0;
        // to find element with maximum difference
        for (int i=0; i<gas.size();i++){
            if ((gas[i]-cost[i]) > (gas[idx]- cost[idx])){
                idx=i;
            }
        }
        // traverse till idx and add them to the same vector
        for (int i=0; i<=idx;i++){
            gas.push_back(gas[i]);
