...
public:
    vector<int> minSubsequence(vector<int>& nums) 
    {
        ...
        2. make a slider say k 
        3. take the slider to divide the array in two 
        4. the first hit will be the ans.
        ...
        */
        ...
        int k=1; // slider
        int sum = nums[0], cur =0 ; // cur- current sum
        ...
        for (int i =k; i<nums.size();i++)
        {
            ...
            cur+= nums[i];
            if (sum>cur && i == nums.size()-1){
                cout<< "sum = \t\t"<< sum << "\t" << cur<< endl;
                cout<<k<< endl;
                // logic to push the vector till the slider and return here
                for(int j=0; j<k ; j++ ){
            ...
            else if (i == nums.size()-1 || sum <cur){
                i = k;
                sum+= nums[k];
                cur =0;
                k++;
                
                if (k==nums.size()){ // for last element to also be in subsequence
                    i--; // [8,8]
                }
