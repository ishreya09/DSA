...
public:
    bool checkDistances(string s, vector<int>& distance) {
        ...
        for(int i=0;i<s.length();i++){
            if (f[s[i]].value==-1)
                f[s[i]].value=i; // first index
        }
        
        for (auto i=f.begin();i!=f.end();i++){
            int d= i->first - 'a';
            cout<< i->first<< "\t"<< d<<"\t"<<i->second.value <<"\t"<<distance[d]+i->second.value+1<<endl;
            if (distance[d]+i->second.value+1>s.length() || s[distance[d]+i->second.value+1] != s[i->second.value] )
                return false;
        }
        return true;
    }
